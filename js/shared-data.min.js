...
window.$CQ=_g.$;
(function(a,b){b(a).on("foundation-contentloaded",function(d){b(".showhide").each(function(){c(b(this))
})
});
b(a).on("change",".showhide",function(d){c(b(this))
});
function c(d){var f=d.data("showhideTarget"),e=d.prop("checked")?d.val():"";
b(f).not(".hide").addClass("hide");
b(f).filter('[data-showhide-target-value="'+e+'"]').removeClass("hide")
}})(document,Granite.$);
(function(a,b){b(a).on("foundation-contentloaded",function(d){c(b(".aem-dialog-dropdown-showhide",d.target))
});
b(a).on("change",".aem-dialog-dropdown-showhide",function(d){c(b(this))
});
function c(d){d.each(function(g,f){var j=b(f).find("coral-select-item[selected]").val();
if(j){var h=b(f).data("aem-dialog-dropdown-showhide-target");
if(h){var e=b(f).parent().parent();
var k=e.find(h);
b(k).each(function(i,l){b(l).not(".hide").addClass("hide");
b(l).filter("[data-showhidetargetvalue='"+j+"']").removeClass("hide")
})
}}})
}})(document,Granite.$);
(function(g,e,i,d){var a="coral-select.core-title-size-default",k="coral-select.core-title-sizes-default",j=".core-title-sizes-allowed coral-checkbox",f="checkboxes.validation.state",h="coral-select.core-title-sizes";
d.on("foundation-contentloaded",function(l){b(g(".aem-dialog-dropdown-showhide",l.target));
g(".cq-dialog-checkbox-showhide").each(function(){c(g(this))
});
Coral.commons.ready(g(j),function(m){if(typeof updateDefaultSizeSelect!=="undefined"){updateDefaultSizeSelect(false)
}})
});
d.on("dialog-ready",function(){if(g("#navImageFlag").is(":checked")){c(g(".cq-dialog-checkbox-showhide-imageOrText"))
}else{c(g(".cq-dialog-checkbox-showhide-imageOrText"))
}});
g(document).on("change",".cq-dialog-checkbox-showhide-imageOrText",function(l){c(g(this))
});
g(document).on("change",".aem-dialog-dropdown-showhide",function(l){b(g(this))
});
function c(l){var o=l.data("cqDialogCheckboxShowhideTarget");
var m=l.prop("checked");
var n=m?l.val():"text";
g(o).not(".hide").addClass("hide");
g(o).filter("[data-showhidetargetvalue='"+n+"']").removeClass("hide").removeClass("hidden")
}function b(l){l.each(function(o,n){var q=g(n).find("coral-select-item[selected]").val();
if(q){var p=g(n).data("aem-dialog-dropdown-showhide-target");
if(p){var m=g(n).parent().parent();
var r=m.find(p);
g(r).each(function(s,t){g(t).not(".hide").addClass("hide");
g(t).filter("[data-showhidetargetvalue='"+q+"']").removeClass("hide")
})
}}})
